...
**Data Collection** - The input file is admittedly not as sophisticated as pulling results from MTG Arena output files, but I choose this approach because I had records from my entire usage across many iterations of the client and old records are automatically discarded by the client. Typing up the input file is alleviated with text editors that feature autocomplete and by working on the task incrementally.
